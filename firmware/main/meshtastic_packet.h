...
#define MESHTASTIC_CHANNEL_NAME_MAX 31
#define MESHTASTIC_CHANNEL_KEY_MAX 32

typedef struct {
    char name[MESHTASTIC_CHANNEL_NAME_MAX + 1];
    uint8_t key[MESHTASTIC_CHANNEL_KEY_MAX];
    uint8_t key_len;
} meshtastic_channel_t;

/* Replace the channel names and keys used by the receive decoder. */
