/*
 * system/crash_dump.c - decode a crash core dump from flash and save it to SD.
 *
...
static size_t save_raw_elf(const char *path)
{
    ...
    if (!chunk) {
        ESP_LOGE(TAG, "no memory for core dump copy buffer");
        fclose(f);
        return 0;
    }
    size_t written = 0;
    ...
    while (left > 0) {
        ...
        size_t n_written = fwrite(chunk, 1, n, f);
        if (n_written != n) {
            ESP_LOGE(TAG, "SD write failed at %u: wrote %u of %u bytes",
                     (unsigned)rel, (unsigned)n_written, (unsigned)n);
            ok = false;
            break;
        }
        rel += n;
    ...
    free(chunk);
    if (fclose(f) != 0) {
        ESP_LOGE(TAG, "SD flush failed for %s", path);
        ok = false;
    }
    return (ok && written == size) ? written : 0;
}
